...
public final class FeatureCommandView {

    private FeatureCommandView() {
    }

    public static Component buildFeatureInfoMessage(
            String name,
            boolean enabled,
            String version,
    ...
    public static Component renderCsvColored(
            List<String> items,
    ...
    public static Component buildLoadedFeaturesOneLine(List<FeatureCommandModel.FeatureListEntry> entries, boolean withVersion) {
        int n = entries.size();
        ...
        for (int i = 0; i < entries.size(); i++) {
            FeatureCommandModel.FeatureListEntry entry = entries.get(i);
